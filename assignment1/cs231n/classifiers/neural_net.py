...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    # 根据scores来计算损失
    # 数值稳定化 矩阵操作时注意加上reshape指定需要的shape，不然非常容易出错
    ...
    # 计算交叉熵 由于log函数不能对0求log，所以要先求出交叉熵，再对其他值写0
    loss = - np.sum(np.log(scores[np.arange(N), y])) / N

    # 这里的0.5一定要，不然求出来的梯度不在 1e-8范围内
    loss += 0.5 * reg * (np.sum(W1 * W1) + np.sum(W2 * W2))
    # 0.01896541960606335
    # 不要 * 0.5 的结果是 1.7985612998927536e-13
    pass
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    # [H * N]  * [N * C] >>> [H * C]
    # print('np.dot(buf_H.T, scores).shape: ', np.dot(buf_H.T, scores).shape)
    # print('N: ', N)
    dW2 = np.dot(buf_H.T, scores) / N + reg * W2
    ...
    # 4*5  * 5*10 >>4*10
    dW1 = np.dot(X.T, buf_relu) / N + reg * W1
